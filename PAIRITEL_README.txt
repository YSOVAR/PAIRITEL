...
Define the central coordinates of your cluster and several other constants in the input.py file.
...
execfile('/data/swolk/kpoppen/Dropbox/MyPython/PAIRITEL/pairitel_pyraf_0.py')

The 'pairitel_pyraf_0.py' script will do some basic operations on the files - it will copy everything you need into the specified resultfolder, and will trim and coordinate-sghift the images.

Step 3

Run 'pairitel_pyraf_1.py' in the pyraf shell. This script will do the aperture photometry for all nights and tell you which night yielded the sharpest H band images (the H band usually has the best contrast), it also tells you in which image the most sources were found by aperture photometry. Look at those images by hand (for example with ds9) and choose one of them as your master image for the source extraction. Save that filename, including the path, in the input.py file.

Then open that image in ds9 (if it's not open yet) and define 5-10 single, bright, but not saturated stars as your stars for the psf fitting. Do this by placing circular regions centered on those stars (the radius does not matter), and save those regions, using decimal fk5 coordinates. Save the filename of that file, including the path, in the input.py file. I usually save mine in the parent folder of the individual nights, so that I can find it easily if I want to check something by hand in ds9.
...
execfile('/data/swolk/kpoppen/Dropbox/MyPython/PAIRITEL/pairitel_pyraf_1.py')
...
execfile('/data/swolk/kpoppen/Dropbox/MyPython/PAIRITEL/pairitel_pyraf_2.py')
...
execfile('/data/swolk/kpoppen/Dropbox/MyPython/PAIRITEL/pairitel_astropy_2.py')
...
execfile('/data/swolk/kpoppen/Dropbox/MyPython/PAIRITEL/pairitel_pyraf_3.py')
...
execfile('/data/swolk/kpoppen/Dropbox/MyPython/PAIRITEL/pairitel_astropy_3.py')
