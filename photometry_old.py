...
def psf_photometry_mosaic(image, ds9=False):
    '''This method performs psf photometry on a mosaicimage
    
    It calls psf_photometry_image on each extension of the image
    and recombines the substracted images to a new mosaic.
    
    input: image: filename
    keyword: ds9=False: will be passed to psf_photometry_image
    '''
    for i in range(1,1+n_extensions(image)):
        imagebi=image+'['+str(i)+']'  #image-Bracket-i        
        psf_photometry_image(imagebi,ds9=ds9)
        #combine all allstar substracted images into a multi-extension fits analogous to the original image
        subtotal= get_next_iraf(image+'.totalsub',suffix='fits') if i==1 else get_last_iraf(image+'.totalsub',suffix='fits')
        if get_last_iraf(imagebi+'.sub',suffix='fits'):
            iraf.imcopy(get_last_iraf(imagebi+'.sub',suffix='fits'),subtotal+'[IM'+str(i)+',append]',verbose=True)
        else:
            #no imagebi+'.sub, e.g. because no pst stars were found
            iraf.imcopy(imagebi,subtotal+'[IM'+str(i)+',append]',verbose=False)
...
def psf_photometry_pairitel(imagebi, satmag, psfstarfile='', ds9=False):
    ...
    iraf.daophot.allstar(imagebi,'default','default','default','default','default',verbose=False)
...
def psf_photometry_image(imagebi,ds9=False):
    '''This method performs psf photometry on a single fits extension
    ...
    input: imagebi: Filename for iraf, e.g. XXX.fits[2]
    keyword: ds9=False: If true the method writes a files with X Y coords of all succesfully fitted sources in the current WCS 
    '''
    iraf.daophot.verify = False
    #needs to be set here because it is different for each amplifier
    ...
    out=iraf.pstselect(imagebi, 'default', 'default', maxnpsf=10, psfrad=0, fitrad=iraf.daopars.psfrad, Stdout=1)
    pstcat=catalog.CartesianCatalog(get_last_iraf(imagebi+'.pst'), type='daophot', RA='XCENTER', DEC='YCENTER')    
    # The following line wirks because Python short-circuits bolean expressions
    # If the first is true, then pst cat is [], so it is important that pstcat['MAG'] is not called
    if ('Total of 0 PSF stars selected' in out[-1]) or (min(pstcat['MAG']) > 24.) or (len(pstcat) < 2):
        #need to find psf stars, try again with less strict criteria
        out2=iraf.pstselect(imagebi, 'default', 'default', maxnpsf=10, psfrad=0, fitrad=iraf.daopars.psfrad/2, Stdout=1)
        if 'Total of 0 PSF stars selected' in out2[-1]:
        #need to find psf stars, try again with less strict criteria
            out3=iraf.pstselect(imagebi, 'default', 'default', maxnpsf=10, psfrad=0, fitrad=15, Stdout=1)
            if 'Total of 0 PSF stars selected' in out3[-1]: raise Exception('No PSF star found!')
    #if only very faint stars are in the pst file then the psf will be bad
    #Likely there no star that can be extracted with resonable accuracies anyway.
    pstcat=catalog.CartesianCatalog(get_last_iraf(imagebi+'.pst'), type='daophot', RA='XCENTER', DEC='YCENTER')
    if min(pstcat['MAG']) < 24.:
        status = filter_pstfile(get_last_iraf(imagebi+'.pst'), get_last_iraf(imagebi+'.mag'))
        if status > 0:
            iraf.daophot.psf(imagebi,'default','default','default','default','default', interactive=False,verify=False,nclean=5)
            #use a smaller radius for nstar and substar to substract the "core only" of close neighbours
            original_psfrad=iraf.daopars.psfrad
            iraf.daopars.psfrad = 15
            iraf.nstar(imagebi,get_last_iraf(imagebi+'.psg'),'default','default','default', verbose = False)
            ...
            #then reset psfrad to original value
            iraf.daopars.psfrad = original_psfrad
            #if no neighbourghs were removed there is no reason to refit the psf
            if 'A total of 0 stars were subtracted' in out[-1]:
                #subtracting 0 stars can corrupt the fits file - delete it
                os.remove(get_last_iraf(imagebi+'.sub',suffix='fits'))
            else:
                #the .sub image should now be free of any neighbours
                iraf.daophot.psf(get_last_iraf(imagebi+'.sub',suffix='fits'), get_last_iraf(imagebi+'.mag'),get_last_iraf(imagebi+'.pst'), get_next_iraf(imagebi+'.psf',suffix='fits'), get_next_iraf(imagebi+'.pst'), get_next_iraf(imagebi+'.psg'), interactive=False,verify=False,nclean=5)
            iraf.daophot.allstar(imagebi,'default','default','default','default','default',verbose=False)
        else:
            print "Not extracting from "+imagebi+'. Ignored due to filter_pst'
        if ds9:
            als=asciitable.read(get_last_iraf(imagebi+'.als'), Reader=asciitable.DaophotReader)
            print_cols(get_next_iraf(imagebi+'.als',suffix='reg'),als['XCENTER'],als['YCENTER'])
    else:
        print "Not extracting from "+imagebi+'. All pst stars are too faint!'

### Mask out holes in middle of brighthest stars
def mask_saturated(imlist, verbose=0, datamax_factor=1.5, mask_radius=3):
    '''Masks out the inner parts of bright stars with negative values
    
    The method takes an image list.
    For each object in the list and each filter the method searches for the image with the shorted exposure time.
    For all images of the same object in the same filter is uses the pixel values in the shorted exposure to predict 
    where the longer images will be overexposed an masks out these regions.
    Usually daophot ignores all pixels with values > iraf.datapars.datamax anyway, but highly saturated stars can
    show holes in the middl with values < iraf.datapars.datamax. These should be masked by this procedure.
    
    input: imlist: List of file names
    keywords: verbose = 0: verbosity level
              datamax_factor = 1.5: Pixels with predicted values > datamax_factor*iraf.datapars.datamax will be masked.
              If iraf.datapars.datamax is very conservative the chip might not be linear above iraf.datapars.datamax.
            but only the holes in the middle of the saturated stars need to be masked. Daophot ignores all pixels
            with values > iraf.datapars.datamax anyway.
          mask_radius = 3 [in pixel]: mask out all pixels within mask_radius around a pixel with 
              predicted values > datamax_factor*iraf.datapars.datamax. This is necessary, if the different exposures 
            are shifted relative to each other.
    '''
    if iraf.datapars.datamax == 'INDEF':
        if verbose > -1: print 'No masking will be applied because iraf.datapars.datamax is INDEF'
    else:
        objlist=np.array([iraf.hselect(im+'[1]','OBJECT', 'yes',Stdout=1)[0] for im in imlist])
        # loop over all objects
        for obj in list(set(objlist[:])):
            if verbose > 1: print "Working on Object "+obj
            thisobjimlist = np.array(np.array(imlist).compress((objlist==obj).flat))
            filterlist = np.array([iraf.hselect(im+'[1]',iraf.datapars.filter, 'yes',Stdout=1)[0] for im in thisobjimlist])
            # loop over all filters for this object
            for filt in list(set(filterlist[:])):
                if verbose > 2: print "    Working on Filter "+filt
                thisfiltimlist = np.array(thisobjimlist.compress((filterlist==filt).flat))
                exptlist = np.array([float(iraf.hselect(im+'[1]',iraf.datapars.exposure, 'yes',Stdout=1)[0]) for im in thisfiltimlist])
                thisfiltimlist = thisfiltimlist[exptlist.argsort()]
                exptlist.sort()
                #loop over all images for this filter and this object
                for image in thisfiltimlist[1:]:
                    # loop over all extensions in that image
                    for ext in range(1,1+n_extensions(image)):
                        imagebi=image+'['+str(ext)+']' 
                        if int(iraf.imstat(imagebi,fields='npix',lower=iraf.datapars.datamax*datamax_factor, Stdout=1)[1]) >= 1:
                            if verbose > 5: print "    Apply mask to "+imagebi
                            maskfile = get_next_iraf(imagebi+'mask',suffix='fits')
                            iraf.imcopy(thisfiltimlist[0]+'['+str(ext)+']', maskfile, verbose=(verbose>6))
                            max_counts = datamax_factor*iraf.datapars.datamax*exptlist[0] / float(iraf.hselect(imagebi, iraf.datapars.exposure, 'yes',Stdout=1)[0])
                            iraf.imreplace(maskfile, -1000,lower=max_counts,radius=mask_radius)
                            iraf.imreplace(maskfile, iraf.datapars.datamax*datamax_factor, lower=-999)
                            iraf.imarith(imagebi,'min',maskfile,image+'[IM'+str(ext)+',overwrite]')
...
def summarise_als(im, outfile):
    '''summarises all .als files of image im to one ASCII file with columns RA, DEC, MAG, MERR
    
    This procedue goes trough all extensions of image im and finds the als file
    with the last version number. It then read these files and produces an
    ASCII text file with four columns: RA, DEX, MAG, MERR
    The keywods exposure time, filter and airmass are kept in the header.
    '''
    def match_source_none(self,pos,data):
        return None
    
    expt = (iraf.hselect(im+'[1]', iraf.datapars.exposure, 'yes',Stdout=1))[0]
    filt = (iraf.hselect(im+'[1]', iraf.datapars.filter, 'yes',Stdout=1))[0]
    air  = (iraf.hselect(im+'[1]', 'AIR', 'yes',Stdout=1))[0]
    obslist = catalog.SimpleCatalog()
    obslist.match_source = match_source_none
    for n in range(n_extensions(im)):
        alsfile = get_last_iraf(im+str(n+1)+'.als')
        if alsfile != None:
            cat = catalog.SimpleCatalog(alsfile, type='my_daophot', fill_values=[('INDEF','nan')])
            cat = cat.where((-2 < cat.SHARPNESS) * (cat.SHARPNESS < 2)) #* (cat.CHI < 5))
            cat.keep_columns(['RA', 'DEC', 'MAG', 'MERR' ])
            obslist.add_catalog(cat.coords[:], cat.data, auto_add_columns=True)
    header=['# File '+im,'# EXPTIME '+expt, '# IFILTER '+filt, '# AIR '+air, 'RA DEC '+filt+'_MAG '+filt+'_ERR']
    if len(obslist) > 0:
        print_cols(outfile, obslist.RA, obslist.DEC, obslist.MAG, obslist.MERR, header = header)
...
def do_aperture_photometry(list_files):
    for datafile in list_files[:]:
        ...
        found_sources_to_ds9(datafile + '.coo.1')
...
def sort_by_sharpness(datalist, nbest):
    ...
    sortsharp = deepcopy(sharpnesses)
    allstuff = zip(sharpnesses, datalist) # sort by first argument in zip.
    allstuff.sort()
    (sortsharp, sortnames) = zip(*allstuff)
    bestn = sortnames[0:nbest]
    bests = sortsharp[0:nbest]
    
    return (sharpnesses, bestn, bests)
